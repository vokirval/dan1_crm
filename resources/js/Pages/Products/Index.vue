...
<template>
  ...
  <Layout>
    ...
    <!-- Таблица -->
    <DataTable paginator showGridlines :value="products.data" :lazy="true" :total-records="products.total"
      :rows="perPage" :rows-per-page-options="[10, 20, 50, 100]" :first="(currentPage - 1) * perPage"
      :sort-field="sortBy" :sort-order="sortDirection === 'asc' ? 1 : -1" @page="onPageChange"
      @error="(e) => console.error('Error in DataTable:', e)" @sort="onSortChange">
      <Column field="id" header="ID" sortable />
      <Column field="name" header="Назва" sortable />
      <Column field="short_name" header="Коротка назва" />
      ...
      <Column header="Статус">
        <template #body="{ data }">
          <div class="flex gap-3 items-center uppercase" v-if="data.status === 'active'">
            <svg style="width:8px; height:8px;" width="12" height="12" viewBox="0 0 24 24" fill="#00FF00"
          ...
          <div class="flex gap-3 items-center uppercase" v-if="data.status === 'inactive'">
            <svg style="width:8px; height:8px;" width="12" height="12" viewBox="0 0 24 24" fill="#FF0000"
              xmlns="http://www.w3.org/2000/svg">
              <circle cx="12" cy="12" r="12" />
            </svg>
    ...
    <Dialog header="Додати продукт" v-model:visible="modalVisible" :modal="true" :style="{ width: '50vw' }">
      <form @submit.prevent="saveProduct">
        <div class="grid gap-4 grid-cols-2">
          ...
          <IftaLabel>
            <InputText v-model="form.short_name" id="prdshortname" class="w-full" />
            <label for="prdshortname">Коротка назва</label>
          </IftaLabel>
          ...
          <IftaLabel>
            <InputNumber v-model="form.discounted_price" locale="ua-UA" :minFractionDigits="2" id="prddp"
              class="w-full" />
          ...
          <IftaLabel>
            <Select v-model="form.category_id" :options="categories" optionLabel="name" optionValue="id" id="prdcat"
              class="w-full" />
            <label for="prdcat">Категорія</label>
          </IftaLabel>

          <IftaLabel>
            <Select v-model="form.status" :options="statuses" optionLabel="name" optionValue="value" id="prdstatus"
              class="w-full" />
            <label for="prdstatus">Статус</label>
          </IftaLabel>

          <IftaLabel>
            <Select v-model="form.type" :options="types" optionLabel="name" optionValue="value" id="prdtype"
              class="w-full" />
        ...
        <Button label="Зберегти" type="submit" class="mt-3 w-full" />
      </form>
    </Dialog>
